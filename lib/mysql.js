...
MySQL.prototype.buildSelect = function(model, filter, options) {
  ...
  if (filter) {
    ...
    if (filter.orderBy) {
      var orderStmtThing = new ParameterizedSQL('ORDER BY ' + filter.orderBy.custom);
      selectStmt.merge(orderStmtThing);
...
MySQL.prototype._buildWhere = function(model, where, tableAlias, aliases, options) {
  ...
  for (var key in where) {
    ...
    // Ignore relation keys
    // if (key in this._models[model].model.relations) continue;
    // Handle and/or operators
    if (key === 'customWhere') {
      var clauses = where[key];
      const clauseKeys = Object.keys(clauses);
      clauseKeys.forEach(function (caluseKey) {
        stmt.merge({
          sql: clauses[clauseKeys],
          params: [],
        });
        ...
        if (relations && relations[modelAndProperty[0]]) {
          ...
          if (alias) {
            ...
            columnNames.push(alias + '.' + modelAndProperty[1] +
              ' as ' + alias.replace('`', '').replace('`', '') + '_orderBy' + modelAndProperty[1]);
          }
        }
      } else {
